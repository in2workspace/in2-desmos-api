...
class BlockchainTxPayloadFactoryTests {
    ...
    void testBuildBlockchainTxPayload_validData_differentHashes_Success() throws Exception {
        ...
        // Assert
        // Check that the previous hash is different from the hash of the data, because it is not the first hash, it is the
        // concatenation of the previous hash and the hash of the data
        StepVerifier.create(resultMono)
                .assertNext(blockchainTxPayload -> {
                    Assertions.assertNotEquals(previousHash,
                            ApplicationUtils.extractHashLinkFromDataLocation(blockchainTxPayload.dataLocation()));
                    try {
                        Assertions.assertEquals(ApplicationUtils.calculateHashLink(previousHash, ApplicationUtils.calculateSHA256(
                                "dataMapString")), ApplicationUtils.extractHashLinkFromDataLocation(blockchainTxPayload.dataLocation()));
    ...
    void testBuildBlockchainTxPayload_invalidData_Failure() throws Exception {
        ...
        // Assert
        // Check that the previous hash is different from the hash of the data, because it is not the first hash, it is the concatenation
        // of the previous hash and the hash of the data
        StepVerifier.create(resultMono)
                .assertNext(blockchainTxPayload -> {
    ...
    void testCalculatePreviousHashIfEmpty_invalidData_Failure() throws Exception {
        ...
        // Check that the previous hash is the hash of the data
        StepVerifier.create(resultMono)
                .verifyErrorMessage("Error creating previous hash value from notification data");
    }
}