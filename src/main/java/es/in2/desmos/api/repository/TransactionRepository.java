...
public interface TransactionRepository extends ReactiveCrudRepository<Transaction, UUID> {
    ...
    @Query("SELECT * FROM transactions ORDER BY created_at DESC LIMIT 2 OFFSET 1")
    Mono<Transaction> findPreviousTransaction();

    @Query("SELECT * FROM transactions WHERE entity_id = :entityId AND status = 'PUBLISHED' ORDER BY created_at DESC LIMIT 1")
